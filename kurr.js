...
› ➥⬣ ${prefix}domain namadomain|ip
› ➥⬣ ${prefix}listgc  (cek id grub)
› ➥⬣ ${prefix}linkgc
├───[ CREATED BY ClyvvXyzz X Vinn ]────
`
let btn = [{
    quickReplyButton: {
            displayText: 'FITUR DIKIT? BELI=!!',
            id: 'subs'
        } 
      
 }]
fatihgans = fs.readFileSync('./lib/assets/logo.jpg'), 
kurr.send5ButImg(m.chat, Anu, ` BOT HOSTING BY RapiiCloud `, fatihgans, btn)
                                          break
...
if (!isCreator) throw  m.reply(`Lu Siapa Ajg`)
        ...
        axios.get(`https://fortunesia.login-server.my.id:2087/json-api/removeacct?api.version=1&username=${uname}`, { headers: { Authorization: "Basic " + Buffer.from("root:pwmu").toString("base64") } })
      ...
      break
                                          case 'subs':
                                          m.reply(`YT : https://youtube.com/channel/UCRnO5l79ENp6SvrTjDVtZRQ`)
                                          break
                                          case 'addpack':
         if(from != "120363043002630241@g.us") return m.reply("maaf su fitur ini dapet di gunakan group tertentu")
        let sUser1 = "root";
        let sPass1 = "pw root";
        let serverName1 = "fortunesia.login-server.my.id:2087";
 ...
 if(from != "120363043002630241@g.us") return m.reply("maaf su fitur ini dapet di gunakan group tertentu")
        function subDomain1(host, ip) {
          return new Promise((resolve) => {
            let zone1 = "cd2d49f07c41d0528a5ee57815f67c02";
            let apiToken1 = "hMPcQ2nHfqVDrXSriaitp43Ir3Hnv7deZTPCFIM8";
            let tld1 = "viral-lagi.my.id";
            axios
              .post(
